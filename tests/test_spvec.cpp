...
template <typename V, typename I, bool One = false>
class Caster : public SparseVector<V, I, One> {
 public:
  typedef SparseVector<V, I, One> base;
...
TEST(SP_VEC, fortran) {
  using sp_t  = SparseVector<double, int, true>;
  const int n = i_rand() + 1;
  auto      v = sp_t(n);
  v.reset_counter();
  ASSERT_EQ(v.size(), 0u);
  // push to n/2 step
  for (int i = 0; i < n; ++i) v.push_back((i_rand() % n) + 1, n / 2);
  ASSERT_LE(v.size(), n);
  const int m = v.size();
  std::cout << "n=" << n << ", m=" << m << '\n';
  // test there should not have overlap indices
  std::vector<bool> tags(n, false);
  for (int i = 0; i < m; ++i) {
    const auto idx = v.c_idx(i);  // direct get c index
    if (!tags[idx]) {
      tags[idx] = true;
      continue;
    }
  ...
  for (int i = 0; i < m2; ++i)
    ASSERT_EQ(v.idx(i) % 2, 1)
        << v.idx(i) << " should not appear in the list!\n";
  // internal test, the sparse tags should be reset all to false!
  const auto &caster  = static_cast<const Caster<double, int, true> &>(v);
  const auto &sp_tags = caster.get_sparse_tags();
  ASSERT_TRUE(std::none_of(sp_tags.cbegin(), sp_tags.cend(), [](bool foo) {
    return foo;
  })) << "sparse tags should be reset to false!\n";
