...
using solver_t = ksp::TGMRESR<prec_t>;

const static char *help =
    "\n"
    "usage:\n"
    "\n"
    " ./demo_tgmres case [options] [flags]\n"
    "\n"
    ...
    " -T|--rtol\n"
    "\trelative tolerance for TGMRES (1e-6) solver\n"
    " -R|--cycle\n"
    "\tcycle for TGMRES (30)\n"
    " -K|--kernel\n"
    "\tinner kernel for TGMRES (30)\n"
...
inline static std::tuple<Options, int, double, bool, int> parse_args(
    int argc, char *argv[]);
...
int main(int argc, char *argv[]) {
  ...
  int     kernel;
  // parse arguments
  std::tie(opts, restart, rtol, symm, kernel) = parse_args(argc, argv);
  ...
  std::cout << (norm2(bb) / norm2(b)) << std::endl;
  return flag;
}
...
inline static std::tuple<Options, int, double, bool, int> parse_args(
  ...
  for (;;) {
    ...
    if (arg == string("-t") || arg == string("--tau")) {
      ...
      tol = std::atof(argv[i]);
    } else if (arg == string("-R") || arg == string("--cycle")) {
      ++i;
      if (i >= argc) fatal_exit("missing GMRES cycle value!");
  ...
  return std::make_tuple(opts, restart, tol, symm, kernel);
}
