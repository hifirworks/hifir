...
#ifndef _HILUCSI_ARCH_LSTRSV_HPP
#define _HILUCSI_ARCH_LSTRSV_HPP
...
class LevelSets {
 public:
  ...
  inline size_type nodes_in_level(const size_type lvl) const {
    hilucsi_assert(lvl < levels(), "%zd exceeds the level number %zd", lvl,
                   levels());
  ...
  inline typename std::enable_if<!IsUpper, T>::type determine_levels(
    ...
    hilucsi_error_if(n != L.ncols(), "must be squared matrix");

    iarray_type lvls(n, 0);
    hilucsi_error_if(lvls.status() == DATA_UNDEF, "memory allocation failed");
  ...
  inline typename std::enable_if<IsUpper, T>::type determine_levels(
    ...
    hilucsi_error_if(n != U.ncols(), "must be squared matrix");
 ...
 protected:
  ...
  inline void _determine_core(const CrsType &A, const iarray_type &lvls) {
    ...
    hilucsi_error_if(_level_start.status() == DATA_UNDEF,
    ...
    hilucsi_assert((size_type)_level_start[max_levels] == n, "fatal!");
    _level_nodes.resize(n);
    hilucsi_error_if(_level_nodes.status() == DATA_UNDEF,
                     "memory allocation failed");
    for (size_type i = 0; i < n; ++i) {
...
class LsSpTrSolver {
 public:
  ...
  inline void optimize() {
    hilucsi_assert(_A_ref, "upset matrix");
#ifdef _OPENMP
...
#endif  // _HILUCSI_ARCH_LSTRSV_HPP
