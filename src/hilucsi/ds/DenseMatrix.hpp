...
#ifndef _HILUCSI_DS_DENSEMATRIX_HPP
#define _HILUCSI_DS_DENSEMATRIX_HPP
...
class DenseMatrix {
 public:
  ...
  inline reference operator()(const size_type i, const size_type j) {
    hilucsi_assert(i < _nrows, "%zd exceeds row bound %zd", i, _nrows);
    hilucsi_assert(j < _ncols, "%zd exceeds column bound %zd", j, _ncols);
  ...
  inline row_iterator row_begin(const size_type row) {
    hilucsi_assert(row < _nrows, "%zd exceeds row bound %zd", row, _nrows);
  ...
  inline typename std::enable_if<Cs::ROW_MAJOR>::type copy_sparse(
    ...
    if (crs.status() == DATA_UNDEF && (crs.nrows() || crs.ncols()))
      hilucsi_warning("input CRS ia a all-zero matrix.");
  ...
  inline typename std::enable_if<!Cs::ROW_MAJOR>::type copy_sparse(
    ...
    if (ccs.status() == DATA_UNDEF && (ccs.nrows() || ccs.ncols()))
      hilucsi_warning("input CCS ia a all-zero matrix.");
...
#endif  // _HILUCSI_DS_DENSEMATRIX_HPP
