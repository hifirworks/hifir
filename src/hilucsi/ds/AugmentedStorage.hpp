...
/*!
 * \file hilucsi/ds/AugmentedStorage.hpp
...
#ifndef _HILUCSI_DS_AUGMENTEDSTORAGE_HPP
#define _HILUCSI_DS_AUGMENTEDSTORAGE_HPP
...
class AugmentedCore {
 ...
 public:
  ...
  inline index_type val_pos_idx(const size_type nid) const {
    hilucsi_assert(!is_nil((index_type)nid), "NIL node detected");
    hilucsi_assert(nid < _val_pos.size(), "invalid nid %zd", nid);
    return _val_pos[nid];
  }
 ...
 protected:
  ...
  inline void _push_back_nodes(const size_type entry, Iter first, Iter last) {
    ...
    for (size_type i = start; i < n; ++i, ++itr) {
      ...
      hilucsi_assert(j < _node_start.size(),
  ...
  inline index_type _start_node(const size_type lid) const {
    hilucsi_assert(lid < _node_start.size(), "%zd exceeds the node start array",
                   lid);
  ...
  inline index_type _next_node(const size_type nid) const {
    hilucsi_assert(!is_nil((index_type)nid), "NIL node detected");
    hilucsi_assert(nid < _node_inds.size(), "invalid nid %zd", nid);
  ...
  inline void _swap_val_pos_exp(const size_type p1, const size_type p2) {
    hilucsi_assert(p1 < _val_pos.size(), "%zd exceeds val_pos size %zd", p1,
                   _val_pos.size());
    hilucsi_assert(p2 < _val_pos.size(), "%zd exceeds val_pos size %zd", p2,
                   _val_pos.size());
  ...
  inline void _rotate_val_pos_left(const size_type n, const size_type src) {
    hilucsi_assert(src < _val_pos_inv.size(),
                   "%zd exceeds val_pos_inv size %zd", src,
                   _val_pos_inv.size());
    hilucsi_assert(src + n <= _val_pos_inv.size(), "%zd exceeds the length",
                   src + n);
    ...
    for (size_type i = 0u; i < n; ++i, ++itr) {
      hilucsi_assert((size_type)*itr < _val_pos.size(),
  ...
  inline void _interchange_ik_head_tail(const size_type i, const size_type k) {
    hilucsi_assert(i < _node_start.size(), "%zd exceeds node_start size %zd", i,
                   _node_start.size());
    hilucsi_assert(k < _node_start.size(), "%zd exceeds node_start_ size %zd",
                   k, _node_start.size());
  ...
  inline void _build_aug(const size_type nlist, const iarray_type &ind_start,
    ...
    for (size_type i = 0u; i < n; ++i) {
      // hilucsi_assert(j < indices.size(), "%zd exceeds indices size", j);
  ...
  inline size_type _nodes_in_list(const size_type i) const {
    hilucsi_assert(i < _node_counts.size(),
...
class AugCRS : public CrsType,
 ...
 public:
  ...
  inline void build_aug(const crs_type &crs, bool clone = false) {
    ...
    hilucsi_error_if(crs.status() == DATA_UNDEF, "undefined data input matrix");
    hilucsi_error_if(
        crs.nrows() + 1u != crs.row_start().size(),
        "row-start array size (%zd) does not agree with nrows (%zd)",
        crs.row_start().size(), crs.nrows());
    hilucsi_error_if(crs.nnz() != crs.col_ind().size(), "inconsistent nnz");
  ...
  inline void build_aug(const ccs_type &ccs) {
    ...
    hilucsi_error_if(ccs.status() == DATA_UNDEF, "undefined data input matrix");
    hilucsi_error_if(
        nrows() + 1u != crs_type::row_start().size(),
        "row-start array size (%zd) does not agree with nrows (%zd)",
        crs_type::row_start().size(), nrows());
    hilucsi_error_if(crs_type::nnz() != col_ind.size(), "inconsistent nnz");
  ...
  void interchange_cols(const size_type i, const size_type k) {
    hilucsi_assert(i < ncols(), "%zd exceeds max ncols", i);
    hilucsi_assert(k < ncols(), "%zd exceeds max ncols", k);
    ...
    for (;;) {
      ...
      if (i_row == k_row) {
        ...
        hilucsi_assert(std::is_sorted(i_col_itr_first, i_col_itr_last),
        ...
        // block above
        hilucsi_debug_code(bool k_col_should_not_exit = !srch_info.first);
        hilucsi_assert(k_col_should_not_exit, "see prefix, failed with row %zd",
                       (size_type)i_row);
        ...
        *i_col_itr_pos              = k;
        ...
        hilucsi_debug_code(i_col_itr_first = crs_type::col_ind_begin(i_row);
                           i_col_itr_last  = crs_type::col_ind_end(i_row));
        hilucsi_assert(std::is_sorted(i_col_itr_first, i_col_itr_last),
        ...
        hilucsi_assert(std::is_sorted(k_col_itr_first, k_col_itr_last),
                       "%zd is not a sorted row", (size_type)k_row);
        // O(log n)
        auto srch_info = find_sorted(k_col_itr_first, k_col_itr_last, i);
        hilucsi_debug_code(bool i_col_should_not_exit = !srch_info.first);
        hilucsi_assert(i_col_should_not_exit, "see prefix, failed with row %zd",
                       (size_type)k_row);
        ...
        *k_col_itr_pos              = i;
        ...
        hilucsi_debug_code(k_col_itr_first = crs_type::col_ind_begin(k_row);
                           k_col_itr_last  = crs_type::col_ind_end(k_row));
        hilucsi_assert(std::is_sorted(k_col_itr_first, k_col_itr_last),
                       "%zd is not a sorted row (after rotation)",
                       (size_type)k_row);
  ...
  inline void defer_col(const size_type from, const size_type to) {
    hilucsi_assert(from < ncols(), "%zd exceeds max ncols", from);
    hilucsi_assert(to < ncols(), "%zd exceeds max ncols", to);
    hilucsi_assert(from < to, "from (%zd) should strictly less than to (%zd)",
                   from, to);
    hilucsi_assert(is_nil(start_col_id(to)), "destination must be empty!");
    ...
    while (!is_nil(col_id)) {
      ...
      hilucsi_assert(itr != crs_type::col_ind_end(row), "fatal");
...
class AugCCS : public CcsType,
 ...
 public:
  ...
  inline void build_aug(const ccs_type &ccs, bool clone = false) {
    ...
    hilucsi_error_if(ccs.status() == DATA_UNDEF, "undefined data input");
    hilucsi_error_if(
        ccs.ncols() + 1u != ccs.col_start().size(),
        "inconsistent sizes between ncols (%zd) and col-start size (%zd)",
        ccs.ncols(), ccs.col_start().size());
    hilucsi_error_if(ccs.nnz() != ccs.row_ind().size(), "inconsistent nnz");
  ...
  inline void build_aug(const crs_type &crs) {
    ...
    hilucsi_error_if(crs.status() == DATA_UNDEF, "undefined data input");
    hilucsi_error_if(
  ...
  void interchange_rows(const size_type i, const size_type k) {
    hilucsi_assert(i < nrows(), "%zd exceeds max nrows", i);
    hilucsi_assert(k < nrows(), "%zd exceeds max nrows", k);
    ...
    for (;;) {
      ...
      if (i_col == k_col) {
        ...
        hilucsi_assert(std::is_sorted(i_row_itr_first, i_row_itr_last),
                       "%zd is not a sorted column", (size_type)i_col);
        // search O(log n)
        auto srch_info = find_sorted(i_row_itr_first, i_row_itr_last, k);
        hilucsi_debug_code(bool k_row_should_not_exit = !srch_info.first);
        hilucsi_assert(k_row_should_not_exit,
        ...
        *i_row_itr_pos              = k;
        ...
        hilucsi_debug_code(i_row_itr_first = ccs_type::row_ind_begin(i_col);
                           i_row_itr_last  = ccs_type::row_ind_end(i_col));
        hilucsi_assert(std::is_sorted(i_row_itr_first, i_row_itr_last),
        ...
        hilucsi_assert(std::is_sorted(k_row_itr_first, k_row_itr_last),
                       "%zd is not a sorted column", (size_type)k_col);
        // search O(log n)
        auto srch_info = find_sorted(k_row_itr_first, k_row_itr_last, i);
        hilucsi_debug_code(bool i_row_should_not_exit = !srch_info.first);
        hilucsi_assert(i_row_should_not_exit,
        ...
        *k_row_itr_pos              = i;
        ...
        // NOTE that the indices should maintain sorted after rotation
        hilucsi_debug_code(k_row_itr_first = ccs_type::row_ind_begin(k_col);
                           k_row_itr_last  = ccs_type::row_ind_end(k_col));
        hilucsi_assert(std::is_sorted(k_row_itr_first, k_row_itr_last),
                       "%zd is not a sorted column (after rotation)",
                       (size_type)k_col);
  ...
  inline void defer_row(const size_type from, const size_type to) {
    hilucsi_assert(from < nrows(), "%zd exceeds max nrows", from);
    hilucsi_assert(to < nrows(), "%zd exceeds max nrows", to);
    hilucsi_assert(from < to, "from (%zd) should strictly less than to (%zd)",
                   from, to);
    hilucsi_assert(is_nil(start_row_id(to)), "destination must be empty!");
    ...
    while (!is_nil(row_id)) {
      ...
      hilucsi_assert(itr != ccs_type::row_ind_end(col), "fatal");
      // assign "defer" column index
      *itr              = to;
...
#endif  // _HILUCSI_DS_AUGMENTEDSTORAGE_HPP
