...
#ifndef _HILUCSI_PRE_AMD_HPP
#define _HILUCSI_PRE_AMD_HPP
...
// HILUCSI extensions
#  define HILUCSI_AMD_CONTROL (AMD_CONTROL + 2)
#  define HILUCSI_AMD_CHECKING AMD_CONTROL
#  define HILUCSI_AMD_SYMM_FLAG (AMD_CONTROL + 1)
...
class AMD {
 public:
  ...
  inline static Int *SuiteSparse_malloc(size_t len, size_t n) {
    return (Int *)std::malloc(len * n);
  }

  inline static void SuiteSparse_free(void *ptr) {
    if (ptr) std::free(ptr);
  }
  ...
  inline static void defaults(double *Control) {
    ...
    if (Control != nullptr) {
      for (i = 0; i < HILUCSI_AMD_CONTROL; i++) {
        Control[i] = 0;
      }
      Control[AMD_DENSE]             = AMD_DEFAULT_DENSE;
      Control[AMD_AGGRESSIVE]        = AMD_DEFAULT_AGGRESSIVE;
      Control[HILUCSI_AMD_CHECKING]  = 1;
      Control[HILUCSI_AMD_SYMM_FLAG] = 0;
    }
  }
...
#undef PRI
  ...
  inline static void postorder(Int nn, Int *Parent, Int *Nv, Int *Fsize,
    ...
    //     Int nels, ff, nchild;
    //     AMD_DEBUG1(("\n\n================================
    //     AMD_postorder:\n")); nels = 0; for (j = 0; j < nn; j++) {
    //       if (Nv[j] > 0) {
    //         AMD_DEBUG1(("" ID " :  nels " ID " npiv " ID " size " ID " parent
    //         " ID
    //                     " maxfr " ID "\n",
    ...
    for (i = 0; i < nn; i++) {
      if (Nv[i] > 0 && Child[i] != EMPTY) {
        ...
        //   AMD_DEBUG1(("bigf " ID " maxfrsize " ID " bigfprev " ID " fnext "
        //   ID
        //               " fprev " ID "\n",
  ...
  static void two(Int n, Int *Pe, Int *Iw, Int *Len, Int iwlen, Int pfree,
     ...
     * MultiFrontal PACKage, both by Davis and Duff).  The routine is based on
     the
     ...
     *
     * (1) dense rows/columns are removed prior to ordering the matrix, and
     placed *	last in the output order.  The presence of a dense row/column
     can *	increase the ordering time by up to O(n^2), unless they are
     removed *	prior to ordering.
     *
     * (2) the minimum degree ordering is followed by a postordering
     (depth-first *	search) of the assembly tree.  Note that mass
     elimination (discussed *	below) combined with the approximate degree
     update can lead to the mass *	elimination of nodes with lower exact
     degree than the current pivot *	element.  No additional fill-in is
     caused in the representation of the *	Schur complement.  The
     mass-eliminated nodes merge with the current *	pivot element.  They are
     ordered prior to the current pivot element. *	Because they can have
     lower exact degree than the current element, the *	merger of two or more of
     these nodes in the current pivot element can *	lead to a single element
     that is not a "fundamental supernode".  The *	diagonal block can have
     zeros in it.  Thus, the assembly tree used here *	is not guaranteed to be
     the precise supernodal elemination tree (with *	"funadmental"
     supernodes), and the postordering performed by this *	routine is not
     guaranteed to be a precise postordering of the *	elimination tree.
     *
     ...
     *
     * Acknowledgements: This work (and the UMFPACK package) was supported by
     the
     ...
     * Nv:  An integer array of size n.  During execution, ABS (Nv [i]) is equal
     to *	the number of rows that are represented by the principal
     supervariable *	i.  If i is a nonprincipal or dense variable, then Nv
     [i] = 0. *	Initially, Nv [i] = 1 for all i.  Nv [i] < 0 signifies that i is
     a
     ...
     * Elen:  An integer array of size n.  See the description of Iw above.  At
     ...
     the number of nonpivotal entries).  Thus Elen [e] < EMPTY. *	Elen (i)
     = EMPTY set when variable i becomes nonprincipal.
     *
     ...
     *	described below).
     *
     * Last: In a degree list, Last [i] is the supervariable preceding i, or
     EMPTY *	if i is the head of the list.  In a hash bucket, Last [i] is the
     hash *	key for i.
     *
     ...
     *
     * Next: Next [i] is the supervariable following i in a link list, or EMPTY
     if *	i is the last in the list.  Used for two kinds of lists: degree
     lists *	and hash buckets (a supervariable can be in only one kind of
     list at a *	time).
     *
     ...
     *	degree lists.
     *
     * W:  An integer array of size n.  The flag array W determines the status
    ...
    typename std::make_unsigned<Int>::type
        hash; /* unsigned, so that hash % n is well defined.*/
     ...
     *                  for the SuiteSparse_long version.  wflg is not allowed
     to
     *                  be >= wbig.
     * we:		W [e]
     ...
     * s:		number of multiply-subtract pairs for LU factorization,
     for the *		    current element me
    ...
    */
    ...
    /*
     * Any parameter (Pe [...] or pfree) or local variable starting with "p"
     * (for Pointer) is an index into Iw, and all indices into Iw use variables
    ...
    // #ifndef NDEBUG
    //   AMD_DEBUG1(("\n======Nel " ID " initial\n", nel));
    //   AMD_dump(n, Pe, Iw, Len, iwlen, pfree, Nv, Next, Last, Head, Elen,
    //   Degree, W,
    //            -1);
    // #endif
    ...
    while (nel < n) {
      ...
      //     AMD_DEBUG1(("\n======Nel " ID "\n", nel));
      //     if (AMD_debug >= 2) {
      //       AMD_dump(n, Pe, Iw, Len, iwlen, pfree, Nv, Next, Last, Head,
      //       Elen, Degree,
      //                W, nel);
      //     }
      // #endif
      ...
      //     + 1)); for (pme = pme1; pme <= pme2; pme++) AMD_DEBUG3((" " ID "",
      //     Iw[pme])); AMD_DEBUG3(("\n"));
      // #endif
      ...
      for (pme = pme1; pme <= pme2; pme++) {
        ...
        // ASSERT(IMPLIES(aggressive, (deg == 0) == (Elen[i] == 1 && p3 ==
        // pn)));
      ...
      //     n)); for (pme = Pe[me]; pme <= Pe[me] + Len[me] - 1; pme++) {
  ...
  inline static Int order(Int n, const Int *Ap, const Int *Ai, Int *P,
    ...
    if ((int)Control[HILUCSI_AMD_CHECKING] == 1) {
      /* check the input matrix:	AMD_OK, AMD_INVALID, or
       * AMD_OK_BUT_JUMBLED */
      status = valid(n, n, Ap, Ai);
    } else {
      if ((int)Control[HILUCSI_AMD_SYMM_FLAG] == 0)
        status = AMD_OK;
      else
    ...
    if (status == AMD_OK_BUT_JUMBLED &&
        (int)Control[HILUCSI_AMD_SYMM_FLAG] != 0) {
