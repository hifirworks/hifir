...
#ifndef _HIF_ALG_CROUT_HPP
#define _HIF_ALG_CROUT_HPP
...
class Crout {
 public:
  ...
  inline void update_compress(CsType &T, PosArray &list,
    ...
    if (T.nnz_in_primary(_step)) {
      ...
      list[first] = _step;
      hif_assert(first != deferred_step(),
  ...
  inline bool update_kappa(const CsType &T, const PosArray &list,
    ...
    index_type idx = list[deferred_step()];
    while (idx != nil) {
      hif_assert((size_type)idx < _step,
  ...
  inline typename std::enable_if<IsSymm>::type update_diag(
      ...
      DiagType &d) const {
    hif_assert(m, "fatal, symmetric block cannot be empty!");
    // get the current diagonal entry
    const auto dk = d[_step];
  ...
  inline void defer_entry(const size_type to_idx, const PosArray &start,
    ...
    while (idx != nil) {
      auto itr = T.ind_begin(idx) + start[idx];
      hif_assert(itr != T.ind_end(idx), "fatal");
 ...
 protected:
  ...
  inline void _load_arow(const ScaleArray &s, const CrsType &crs_A,
    ...
    // ut should be empty
    hif_assert(ut.empty(), "ut should be empty while loading A");
  ...
  inline void _load_acol(const ScaleArray &s, const CcsType &ccs_A,
                         ...
                         SpVecType &l) const {
    // runtime
    hif_assert(l.empty(), "l should be empty while loading A");
    const size_type defer_thres = deferred_step();
