...
#ifndef _HIF_DS_ARRAY_HPP
#define _HIF_DS_ARRAY_HPP
 ...
 public:
  ...
  explicit Array(const size_type n)
      : _data(new (std::nothrow) value_type[n]),
        ...
        _counts(new _RefCount()) {
    // handle overflow
    hif_assert(_data, "memory allocation failed");
    if (!_data) {
      _size = _cap = 0u;
      _status      = DATA_UNDEF;
    }
...
#undef RESET_ARRAY
  ...
  inline reference operator[](const size_type i) {
    hif_assert(i < _size, "%zd exceeds the size bound %zd", i, _size);
    return _data[i];
  }
  ...
  inline reference at(const size_type i) {
    hif_error_if(i >= _size, "%zd exceeds the size bound %zd", i, _size);
    return _data[i];
  }
  ...
  inline void resize(const size_type n, bool presv = true) {
    hif_assert(_status != DATA_WRAP, "cannot resize external data");
    // quick return
    if (n <= _cap) {
      _size = n;
      return;
    ...
    hif_assert(data, "memory allocation failed");
    if (!data) {
      // call destructor
      this->~Array();
      _counts = new _RefCount();
  ...
  inline void reserve(const size_type n) {
    hif_assert(_status != DATA_WRAP, "cannot call reserve for external data");
    if (_cap >= n) return;
