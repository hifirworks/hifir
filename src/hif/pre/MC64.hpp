...
#ifndef _HIF_PRE_MC64_HPP
#define _HIF_PRE_MC64_HPP
...
void HIF_FC(mc64i, MC64I)(int *);
void HIF_FC(mc64id, MC64ID)(int *);

// routine for computing the matching and scaling
void HIF_FC(mc64a, MC64A)(int *, int *, int *, int *, int *, float *, int *,
                          int *, int *, int *, int *, float *, int *, int *);
void HIF_FC(mc64ad, MC64AD)(int *, int *, int *, int *, int *, double *, int *,
...
inline void set_default_controls(int *controls) {
  IsDouble ? HIF_FC(mc64id, MC64ID)(controls) : HIF_FC(mc64i, MC64I)(controls);
}
...
class MC64 {
 ...
 public:
  ...
  inline static void set_default_controls(par_type &p, const int verbose) {
    hif::template set_default_controls<_IS_DOUBLE>(p.data());
    if (verbose == VERBOSE_NONE)
      p[0] = p[1] = -1;
    else if (hif_verbose2(PRE, verbose))
      p[2] = 1;
#ifdef NDEBUG
    p[3] = 1;
#endif
  ...
  inline static void do_matching(const int verbose, crs_type &B,
    ...
    do {
      Array<int> iw(5 * n);
      hif_error_if(iw.status() == DATA_UNDEF, "memory allocation failed");
      Array<value_type> dw(3 * n + nnz);
      hif_error_if(dw.status() == DATA_UNDEF, "memory allocation failed");
      par_type info, icntl;
