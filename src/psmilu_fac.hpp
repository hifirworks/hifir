...
#  define _SHOW_MAX_MIN_MINABS(__v)                                         \
  ...
  // create l and ut buffer
  SparseVector<value_type, index_type, ONE_BASED> l(A.nrows()), ut(A.ncols()),
      l2(A.nrows()), ut2(A.ncols());
  IndexValueArray<value_type, index_type> pvt_entries(
      std::max(A.nrows(), A.ncols()));  // C based

  std::vector<bool> pvt_tags(A.nrows(), false);
  ...
  for (; step < m; ++step) {
    ...
    if (pvt) {
      size_type pvt_entry;
      step.fetch_A_row(s, A_crs, t, p[step], q, ut2);
      step.fetch_A_col<IsSymm>(s, A_ccs, t, p, q[step], m, l2);
      step.find_pvt_entries<IsSymm>(ut2, l2, m, d, pvt_tags, tau_d,
                                    pvt_entries);
      bool fall_back = true;
      if (pvt_entries.size())
        for (size_type j(0); j < pvt_entries.size(); ++j) {
          pvt_entry = pvt_entries.c_idx(j);
          // compute kappa ut
          update_kappa_ut(step, U, kappa_ut, pvt_entry);
          // then compute kappa for l
          update_kappa_l<IsSymm>(step, L, kappa_ut, kappa_l, pvt_entry);
          pvt = std::abs(kappa_ut[step]) > tau_kappa ||
                std::abs(kappa_l[step]) > tau_kappa;
          if (pvt) continue;
          fall_back = false;
      ...
      // defer bad column to the end for U
      U.interchange_cols(step, pvt_entry);
      // defer bad row to the end for L
      L.interchange_rows(step, pvt_entry);
      // udpate p and q; be aware that the inverse mappings are also updated
      p.interchange(step, pvt_entry);
      q.interchange(step, pvt_entry);
      // update diagonal since we maintain a permutated version of it
      std::swap(d[step], d[pvt_entry]);
      ++local_interchanges;
      if (IsSymm) internal::update_L_start_symm(L, m, L_start);
      if (!fall_back) pvt_tags[pvt_entry] = true;
    }
