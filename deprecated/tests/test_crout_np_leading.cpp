...
TEST(LU, c) {
  ...
  std::cout << "begin crout update on leading block " << m << "\n\n";
  for (; (int)crout < m; ++crout) {
    ...
    crout.update_L_start<false>(L2, m, L_start);
    ...
    crout.compute_l<false>(s, A_ccs, t, p, crout, m, L2, L_start, d2, U2, l);
    ...
    crout.update_B_diag<false>(l, ut, m, d2);
    // sort indices
  ...
  for (int i = 0; i < m; ++i) std::cout << U_start[i] << '\n';
